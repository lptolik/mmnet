...
Different microbiomes with various states (obese or lean) have different KO abundances. We provides function \textit{constructSSN} to strip the reference netwok to obtain the state specific metabolic network for each microbiome. The KO abundance was add to the network as a nodes attributes automatically.
