...
This package addresses the microbiome metabolic network construction and analysis. Below we lists the main features of \emph{mmnet}: 
  ...
  \begin{itemize}
    \item Reference network construction and updating
    \item Functional annotation of microbiome with the Metagenomics RAST sever and annotation data loading
    \item Parsing annotaion data to obtain the KO information and constructing State Specific Network (SSN)
    \item SSN topological ananlysis and comparison
    \item Reference metabolic network, SSN and compared network visualization 
  \end{itemize}

\subsection{Installation}

\emph{mmnet} requires that \emph{KEGGREST}, \emph{igraph}, \emph{Biobase}, \emph{XML}, \emph{RCurl}, \emph{RJSONIO}, \emph{stringr}, \emph{ggplot2} are installed. These should be installed automatically when you install \emph{mmnet}. To install \emph{mmnet}, source biocLite from bioconductor:
...
We will demonstrate some of the functions in \emph{mmnet}. A public dataset which have been annotated in MG-RAST was taken for demenstration purposes since the time Sequnce annotation taken on MG-RAST will range between a few hours and weeks. There is only a  simple code example for microbiome functional annotation.

The test data is consisting of several microbiomes from twin pairs and their mothers and available in MG-RAST. 

\subsection{Reference metabolic network}

This package takes KEGG database to annotate KOs with metabolic reactions which is the basis representation for all proteins and functional RNAs corresponding to KEGG pathway nodes, BRITE hiearchy nodes, and KEGG module nodes to annotation the microbiome.  The KEGG metabolic pathway is the best organized part of KEGG PATHWAY database, and also is a network of KO-KO  relations \citep{kanehisa2000kegg}. It is composed of KO, substrate and product of KO, and have been applied widely to create genome-scale metabolic networks of various microbial species \citep{oberhardt2009applications}. 

An initial reference data named \textit{RefDbcache.rda} was saved in "data" subdirectory of \emph{mment}, that contains KOs and anntated with a metabolic reaction. Morever, an KO based reference metabolic network was also saved in \textit{RefDbcache.rda}, where nodes represent KOs and a directed edge from KO1 to KO2 indicates that a product metabolite of a reaction catalyzed by a KO is a substrate metabolite of a reaction catalyzed by another KO. Howerver, KEGG database updated frequently, reference data can be updated by function \textit{updateMetabolicNetwork} manually and saved in the directory user specified. 
...
##
   names(RefDbcache)
## KO information, metabolites of annotated reation is 
...
\subsection{Microbiome functional annotation}
...
To estimate the KO abundance, \textit{estimateKOAbundance} function should be called for each sample.

<<estimate,echo=TRUE>>==
 KOAbund <- lapply(mgp10Anno,estimateKOAbundance)
 summary(KOAbund)
 lapply(KOAbund,head,7)
@
...
## ko union among all microbiome, abundance of absent KOs is set to 0
 kos <- lapply(KOAbund,names)
 nodes <- unique(unlist(kos))
 diff.ko <- lapply(kos,function(x)setdiff(nodes,x))
 extend.abund <- as.data.frame(mapply(function(x,y)c(x,y),KOAbund,
  lapply(diff.ko,function(x){abund=rep(0,length(x));names(abund)=x;return(abund)})))
 ## sort the abundance according to a sample
 kos.index <- as.data.frame(apply(as.data.frame(mapply(function(x,y)c(x,y),
  kos[-1],diff.ko[-1])),2,function(x)match(nodes,x)))
 extend.abund[-1] <- mapply(function(x,y)x[y],extend.abund[-1],kos.index)
 extend.abund <- (sapply(extend.abund,function(x)x/sum(x)))
 cor(extend.abund,method="pearson")
@
Appanrently, KO abundance across all samples was highly concordant (R>0.77)

\subsection{State specific metabolic network constructing and analysis}
...
Different microbiomes with various states (obese or lean) have different KO abundances. We provides function \textit{stripSSN} to strip the reference netwok to obtain the state specific metabolic network for each microbiome. The KO abundance was add to the network as a nodes attributes.

<<strip-network,echo=TRUE>>==
  SSN <- lapply(KOAbund, function(x)stripSSN(RefDbcache$network, x))
  g <- SSN[[3]]
  summary(g)
...
<<analyzeSSN, echo=TRUE, fig=TRUE>>==
 topo.feature <- analyzeSSN(g, Scatterplot=TRUE, .properties=list("betweenness", 
                  "degree", "neighborhoodCon", "topoparam"))
...
To compare the abundance of KOs across various samples, we take two strategies odds ratio and difference rank of each KO to identify the abnormal KO (enrich or deplete). It will construct a compararive network, which nodes of these abnormal KO with the specific color by calling function \emph{compareKOState}, it also can plot the compared network.

\begin{figure}[!htbp]
  \begin{center}
<<compareKOState, echo = TRUE, fig = TRUE>>==
 comparedNet <- compareKOState(KOAbund, method="OR", cutoff = c(0.5, 2))
 summary(comparedNet)
@
