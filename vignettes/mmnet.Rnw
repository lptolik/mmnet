...
This package addresses the microbiome metabolic network construction and analysis. Below we lists the main features of \emph{mmnet}: 
  ...
  \begin{itemize}
    ...
    \item Constructing the reference metabolic network
...
\emph{mmnet} requires that \emph{KEGGREST}, \emph{igraph}, \emph{Biobase}, \emph{XML}, \emph{RCurl}, \emph{RJSONIO}, \emph{stringr}, \emph{ggplot2}, \emph{biom} are installed. These should be installed automatically when you install \emph{mmnet}. To install the latest devel version \emph{mmnet}, use the devel version of bioconductor:
...
To estimate the abundance of enzymatic genes, \textit{estimateAbundance} function should be called for each sample.
...
Different microbiomes with various states (obese or lean) have different KO abundances. We provides function \textit{stripSSN} to strip the reference netwok to obtain the state specific metabolic network for each microbiome. Both output from \textit{estimateKOAbundance} and a given Biological Observation Matrix (BIOM) \citep{mcdonald2012biological} format could be taken by this function. The BIOM file format (canonically pronounced biome) is designed to be a general-use format for representing biological sample by observation contingency tables. More details on BIOM file can be found in \url{http://biom-format.org/}  R package \emph{biom}. There is also a example BIOM file in directory "inst/extdata" of \emph{mmnet}. The KO abundance was add to the network as a nodes attributes automatically. 

<<strip-network,echo=TRUE>>==
  ssn <- lapply(KOAbund, stripSSN)
  g <- ssn[[3]]
  summary(g)
  abund <- get.vertex.attribute(g,"abundance",index=V(g))
  summary(abund)

  ## BIOM file represents enzyme abundance
  if (require(biom)){
  abund2 <- read_biom(system.file("extdata", "enzymeAbund.biom", package = "mmnet"))
  subnet <- stripSSN(abund2)
}
@
